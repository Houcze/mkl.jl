function eigvals(x)
    ...
    c_eigvals = ccall((:eigvals, "./lib/libjulia.dll"), Ptr{Cdouble}, (Ptr{Cdouble}, Cint), x, dim)
...
function eigvecs(x)
    ...
    dim = Int32(sqrt(size(x, 1)))
    c_eigvecs = ccall((:eigvecs, "./lib/libjulia.dll"), Ptr{Cdouble}, (Ptr{Cdouble}, Cint), x, dim)
...
function matmul(A, B)
    ...
    A = reshape(A, :)
    B = reshape(B, :)
    c_result = ccall((:matmul, "./lib/libjulia.dll"), Ptr{Cdouble}, (Ptr{Cdouble}, Ptr{Cdouble}, Cint, Cint, Cint), A, B, m, n, k)
...
function cholesky(A)
    ...
    try
        c_result = ccall((:cholesky, "./lib/libjulia.dll"), Ptr{Cdouble}, (Ptr{Cdouble}, Cint), A, dim)
...
function inverse(A)
    ...
    dim = Int32(sqrt(size(A, 1)))
    c_result = ccall((:inverse, "./lib/libjulia.dll"), Ptr{Cdouble}, (Ptr{Cdouble}, Cint), A, dim)
    B = unsafe_wrap(Array, c_result, (dim, dim), own=true)
    return copy(B)
end
